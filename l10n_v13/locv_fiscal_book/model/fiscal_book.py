...
class FiscalBook(models.Model):
   ...
   # company_id = fields.Many2one('res.company', 'Compañia', help='Company', )
    ...
    def update_book_issue_invoices(self, fb_id):
        ...
        for invoice in issue_inv_ids:
            # tasa = self.obtener_tasa(invoice) if (invoice.currency_id.name == "USD" or invoice.currency_id.name == "EUR") else 1
            ...
            # a_pagar = amount_total #invoice.amount_total
    ...
    #    ait_obj = self.env['account.invoice.tax']
        ...
        for fbl in self.fbl_ids:
            ...
            if fbl.iwdl_id.invoice_id:
                ...
                #tasa = self.obtener_tasa(fbl.invoice_id) if (fbl.invoice_id.currency_id.name == "USD" or fbl.invoice_id.currency_id.name == "EUR") else 1
                sign = 1 if fbl.doc_type != 'N/CR' else -1
  ...
  #      ait_obj = self.env['account.invoice.tax']
        ...
        for fbl_brw in self.fbl_ids:
            ...
            if fbl_brw.iwdl_id.invoice_id:
                ...
                #tasa = self.obtener_tasa(fbl_brw.invoice_id) if (fbl_brw.invoice_id.currency_id.name == "USD" or fbl_brw.invoice_id.currency_id.name == "EUR") else 1
                for line in fbl_brw.invoice_id.invoice_line_ids:
                    base_amount += self._convert(line.price_subtotal * sign, fbl_brw.invoice_id) #line.price_subtotal * sign * tasa
                    tax_amount += self._convert((line.price_total-line.price_subtotal) * sign, fbl_brw.invoice_id) #(line.price_total-line.price_subtotal) * sign * tasa #
 ...
 #       ait_obj = self.env['account.invoice.tax']
            ...
            if fbl.iwdl_id.invoice_id:
                ...
                for ait in fbl.iwdl_id.tax_line:
                    ...
                    if busq:
                        ...
                        empty_tax += 'Factura: {} - Producto: {} \n'.format(fbl.invoice_id.name, line.product_id.name)
                ...
                for line in fbl.invoice_id.invoice_line_ids:
                    ...
                    if busq:
                        ...
                        line_taxes.update({'fb_id': fb_id,
                                           'fbl_id': fbl.id,
                                           'base_amount': self._convert(base, fbl.invoice_id), #base
                                           'tax_amount':  self._convert(amount, fbl.invoice_id), #amount, #
                                           'name': name,
                                           'type': fiscal_book.type
        ...
        if empty_tax:
            raise UserError(_('Los productos de las siguientes facturas no poseen impuestos que aplica sobre el importe base: \n' + empty_tax))
        ...
        if invoice.exchange_rate:
            ...
            if not tasa1:
                raise exceptions.except_orm("Advertencia!",
                                        "No hay referencia de tasas registradas para moneda %s en la fecha igual o inferior de la factura %s" %(invoice.currency_id.name, invoice.name))
            tasa = tasa1.rate
        return tasa

    def _convert(self, amount, invoice):
        if invoice.currency_id != invoice.company_currency_id:
            rate = invoice.company_currency_id._get_conversion_rate(invoice.currency_id, invoice.company_currency_id, invoice.company_id, invoice.date)
