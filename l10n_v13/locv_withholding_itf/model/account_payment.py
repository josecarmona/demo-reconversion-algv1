...
class AccountPaymentInnerit(models.Model):
    ...
    # payment_id_igtf_check = fields.Boolean('Check', compute='_compute_payment_id', store=True)

    # @api.depends('move_itf_id.line_ids')
    # def _compute_payment_id(self):
    #     for payment in self:
    #         if not payment.payment_id_igtf_check:
    #             for line in payment.move_itf_id.line_ids:
    #                 if not line.payment_id:
    #                     line.update({'payment_id': payment.id})
    #             payment.payment_id_igtf_check = True
    ...
    def post(self):
        ...
        for pago in self:
            ...
            if res and itf_bool and type_bool:
    ...
    def register_account_move_payment(self):
        ...
        porcentage_itf = self.journal_id.wh_porcentage
        ...
        if self.currency_id.id == 3:
            currency = False
            amount_currency = 0.0
            amount_itf = round(float(self.amount) *
                               float((porcentage_itf / 100.00)), 2)
        else:
            currency = self.currency_id.id
            amount_currency = self.amount
            tasa = self.env['res.currency'].search([('id', '=', 3)]).rate
            if tasa:
                amount_itf = round(float(self.amount*tasa)
                                   * float((porcentage_itf / 100.00)), 2)
            else:
                raise UserError(
                    _('Por favor Registrar la tasa para poder hacer la respectiva conversion y poder continuar'))
        self.move_advance_ = {
        ...
        move_line_id1 = move_line_obj.with_context(
            check_move_validity=False).create(asiento)
        asiento['amount_currency'] = amount_currency
        asiento['account_id'] = self.journal_id.account_wh_itf_id.id
        asiento['credit'] = 0.0
    ...
    def check_payment_type(self):
        ...
        for pago in self:
            ...
            if type_payment == 'outbound' or type_payment == 'transfer':
                type_bool = True
        return type_bool
            ...
            if pago.state == 'cancelled':
                for move in pago.move_itf_id:
                    ...
                    if len(move_reverse) == 0:
                        raise UserError(
                            _('No se reversaron los asientos asociados'))
        return res
...
class AccountMoveInnerit(models.Model):
    ...
    def assert_balanced(self):
        if not self.ids:
            return True
        mlo = self.env['account.move.line'].search(
            [('move_id', '=', self.ids[0])])
